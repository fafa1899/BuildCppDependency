...
CMakeArgs="-DBUILD_SHARED_LIBS=ON -DFT_DISABLE_BZIP2=ON"
TargetFile="${InstallDir}/lib/libfreetype.so"

# 组装要传递给 build-common.sh 的参数
common_args=()
common_args+=("-installdir" "$InstallDir")
common_args+=("-requiredlibs" "zlib libpng")
if [ "$FORCE" = true ]; then
  common_args+=("-force")
fi
